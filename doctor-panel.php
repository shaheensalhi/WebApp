...
if (isset($_GET['accept'])) {
    ...
    if ($sql) {
        // Retrieve the name and email of the patient from the database
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            echo "<script>alert('Failed to retrieve patient information.');</script>";
        }
    }
}
...
if (isset($_GET['cancel'])) {
    ...
    if ($query) {
        // Retrieve the name and email of the patient from the database
        $var = "SELECT fname, lname, email FROM appointment WHERE AppID = '$AppID'";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            echo "<script>alert('Your appointment was successfully canceled.');</script>";
            sendEmail($name, $email, $subject, $message);
        } else {
            echo "<script>alert('Failed to retrieve patient information.');</script>";
        }
...
if (isset($_GET['prescribe'])) {
    ...
    if ($query) {
        // Retrieve the patient's email from the appointment table
        $query2 = mysqli_query($con, "SELECT email FROM appointment WHERE AppID = '$AppID'");
        $row = mysqli_fetch_array($query2);
        $email = $row['email'];

        // Send email to patient with prescription details
        $subject = 'Your Prescription from Dr. '. $doctor;
        $message = 'Prescription details:<br>Disease: '. $disease. '<br>Allergy: '. $allergy. '<br>Prescription: '. $prescription. '<br><br> Please follow the instructions carefully and contact us if you have any questions.<br><br>Best regards';
        sendEmail($name, $email, $subject, $message);
